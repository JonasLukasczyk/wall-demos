...
import { float, If, PI, color, cos, instanceIndex, Loop, mix, mod, sin, Fn, uint, uniform, uniformArray, hash, vec3, vec4, storage, storageObject, positionLocal, Discard, length} from 'three/tsl';
...
async function animate() {
  ...
  if(lambda.value>=1){
    // console.log('iterate')
    lambda.value = 0;
    await renderer.computeAsync( iterateCompute );
  }
